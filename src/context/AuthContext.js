        ...
        try {
            ...
            if (profileData) {
                ...
                if (data && data.last_room_id) {
                    setUser(prev => {
                        if (!prev) return prev; // Guard: don't update if user is null
                        return { ...prev, last_room_id: data.last_room_id };
                    });
                }
            }
        } catch (e) {
        ...
        const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
            if (firebaseUser) {
                ...
                try {
                    ...
                    const dbSessionId = localStorage.getItem('rtm_db_session_id');
                    ...
                    if (response.ok) {
                        ...
                        if (sessionData && sessionData[0] && sessionData[0].id) {
                            ...
                            localStorage.setItem('rtm_db_session_id', sessionData.id);
                        }
                    }
                } catch (e) {
